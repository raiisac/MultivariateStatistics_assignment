...
The performance measures (column 3 and 4 in Table \@ref(tab:CFA1performancecorrBI)) show a good fit and all residual correlations are between -0.05 and 0.05 (the simpler model had `r n_simple` (`r round(n/(9*8/2)*100,2)`%) residual correlations between -0.05 and 0.05). We shall thus keep this model as the final model.
...
When comparing the diagonal elements of R(U, T) and R(U*,T*), we see that the first two canonical correlations are stable, as 0.482 is almost equal to 0.468, and 0.244 is almost equal to 0.215. In particular, the first one is stabler than the second. In comparison, the last two pairs of canonical correlations are much less stable. 
