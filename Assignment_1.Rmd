...
#or try modification index
#modificationIndices(fit1corr) --> should add A_organic1 ~~   A_packaging2
```
...
The performance measures (Table \@ref(tab:CFA1performancecorrBI)) show a good fit and most residual correlations are between -0.05 and 0.05 (only `r n` have an absolute value that is slightly higher with a maximum of `r round(max(abs(cor_table)),3)`). We shall thus keep this model as the final model.
