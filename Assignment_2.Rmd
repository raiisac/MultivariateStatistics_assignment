...
#compare models
do.call(rbind, Map(data.frame, A=names, B=ari_values, C=bic))
...
do.call(rbind, Map(data.frame, A=names2, B=ari_values2, C=bic2))
...
mapClass(hddc.out$class,train.target) 
...
points(Z[train.target==0,],col="red")
points(Z[train.target==1,],col="blue")
points(Z[train.target==7,],col="green")
...
When fitting the common dimnsion hddc models on the training data, with a 3-cluster  model specified as "AkjBkQkD" or "AkjBQkD", dimensions from 2 to 6, we can see that the best performing model is "AkjBQkD" with 4 dimensions (ARI = 0.937). It has a BIC of -139,519,143, which is however not the lowest value among all models. The model "AkjBQkD" with 2 dimensions has a BIC of -142,899,972 and an ARI of 0.921.
 
In addition, when fitting the Mclust models "VVE","VEV","EVV","VVV" on the first 2 to 6 unstandardized principal components, we can see that the best performing model is "VVE" fitted on 6 principal components (ARI = 0.9306558). Comparatively, the best hddc model outperforms the Mclust model in terms of ARI. Also, the BIC values of Mclust are all much higher than those of hdcc models. 
 
When visualizing the predicted class labels for the best clustering model in the space of the first two principal components, we can see that the space is divided into 3 parts colored each in red, green and blue. There is little overlap among the clusters, providing a visual illustration that the clustering worked well. On the other hand, when visualizing the actual class labels in the space of the first two principal components, we see some clear overlaps between two clusters.
...
#The results show that stress-1 is lowest using ordinal MDS.(0.089)
#Since it is between 0.05 and 0.1, it is a fair fit.
#Another two optiosn to better assess goodness of fit, is through stress norm and permutation test.
...
plot(m3,plot.type="conf",main="configuration ordinal MDS",pch='',label.conf = list(label = TRUE, pos = 3, col = 1, cex = 1.5))
...
mean(rstress.m3)-2*sd(rstress.m3)
#permutation test
set.seed(1)
...
#print coefficients
round(coef(bimorse),3)

#print R-square of regressions
round(bimorse$R2vec,3)
...
plot(bimorse, main = "Biplot Vector Representation", vecscale = 0.8, 
  xlim = c(-1.5, 1.5), ylim=c(-1.5,1.5), vec.conf = list(col = "brown"), pch ='', cex = 0.5)
```
The results show that stress-1 is lowest using ordinal MDS.(0.089) Since it is between 0.05 and 0.1, it is a fair fit. Another two options to better assess goodness of fit, are stress norm and permutation test. we can see that the stress-1 obtained on the real datadaset (0.089) is much lower than the mean of the stress random data. Similarly, it is much lower than the mean of the distribution from the stress permulated data. We can thus conclude that it is a satisfactory fit.

From the jackknife plot, we can see that the solution is quite stable, because the positions of the points do not change much. The stability measure by jackmds is 0.998, also confirming that the solution is very stable.

When projecting the external variables height and width in the MDS solution, we can see from the coefficients and the plot that if D1 increases one unit, the predicted average height of rectangles will decrease 1.623 standard deviations, and if D2 increases one unit, the predicted average height will increase 0.334 standard deviations. On the other hand,  if D2 increases one unit, the predicted average width of rectangles will increase 2.778 standard deviations, and if D1 increases one unit, the predicted average width will increase 0.096 standard deviations. We also see that D1 and D2 account for 98.7% of the variation in the height around its mean, and 94.5% of the variation in the width around its mean. The correlations of D1 and D2 with the external variables show that the first dimension has a strong negative correlation with the height (cor=-0.987), and that the second dimension has rather strong positive correlation with the width (0.970).
