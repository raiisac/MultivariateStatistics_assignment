...
```{r load2, include=TRUE, message=FALSE, warning=FALSE, cache = TRUE}
...
```{r showhddc, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE}
#compare models
...
```{r showmclust, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r bestplots, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "A comparison of the best models with the true, actual labels."}
par(mfrow = c(1,2))
...
# Task 3
At first, we import the related data files and libraries to the R environment. 
...
Multidimensional scaling on a symmetric dissimilarity matrix is being used in the following section. The function smacofSym() is used to demonstrate to fit the model with 4 levels which are ratio, interval, orfinal and spline respectively.
...
The following shows the Residual Plot and Shepard Diagram with respect to Ordinal MDS and Spline MDs. 

```{r shephard, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Residual plots and Shepard diagrams for ordinal and spline MDS."}
par(mfrow = c(2,2))
...
```{r ordinalMDS, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Configuration ordinal MDS."}
...
In order to check the goodness of fit whether it is in a satisfactory level, stress norms and a permutation test are used.
...
distr_stress <- mean(rstress.m3) - 2 * sd(rstress.m3)
#permutation test
set.seed(1)
...
```{r distrstress, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Distribution of stress."}
...
```{r stability, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Jackknife plot and biplot."}
...
plot(bimorse, main = "Biplot Vector Representation", vecscale = 0.8, 
  xlim = c(-1.5, 1.5), ylim = c(-1.5, 1.5), vec.conf = list(col = "brown"), 
  pch = '', cex = 0.5)
```


```{r table3, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "A comparison of the best models with the true, actual labels."}
tab <- bimorse_coef %>%
  rbind(bimorse_corr) %>%
  rbind(bimorse_r2) 
...
tab %>%
  kable(booktabs = TRUE,
        caption = "MDS solution") %>%
  group_rows(1,2, group_label = "Coefficients") %>%
  group_rows(3,4, group_label = "Correlations") 
```

The results show that stress-1 is lowest using ordinal MDS.(0.089) Since it is between 0.05 and 0.1, it is a fair fit. Another two options to better assess goodness of fit, are stress norm and permutation test. we can see that the stress-1 obtained on the real dataset (0.089) is much lower than the mean of the stress random data. Similarly, it is much lower than the mean of the distribution from the stress permuted data. We can thus conclude that it is a satisfactory fit.
