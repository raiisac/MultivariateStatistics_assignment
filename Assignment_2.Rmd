...
qda_analysis <- function(data, group = "Private"){
  qda.out <- qda(
    formula(paste0(group, "~", paste(colnames(data)[colnames(data) != group], 
                                     collapse = "+"))),
    data = data,
...
#LDA
lda_results <- testlist %>% map(function(x) lda_analysis(x,"Private"))
  
#QDA
qda_results <- testlist %>% map(function(x) qda_analysis(x,"Private"))
