...
# Task 3
```{r load3, include=FALSE, message=FALSE, warning=FALSE, echo=FALSE}
if (file.exists(find_root_file("data/rect_constr.csv", 
     criterion = has_file("MultivariateStatistics_assignment.Rproj")))) {
...
```{r table3, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE}
tab <- bimorse_coef %>%
  rbind(bimorse_corr) %>%
  rbind(bimorse_r2) 
...
```{r include=TRUE, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
Wilks_manova <- function(data, group = "Private"){
  lm.out <- lm(cbind(
    sapply(X = colnames(data)[colnames(data) != group], 
           FUN = function(x) get(x)))~ 
